...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return ((Yii::app()->getModule('user')->isAdmin())?array(
			...
			array('createtime, lastvisit, superuser, status, companyid', 'numerical', 'integerOnly'=>true),
		):((Yii::app()->user->id==$this->id)?array(
			array('username,companyid, email', 'required'),
                     // array('companyid',  'integerOnly'=>true),
			array('username', 'length', 'max'=>20, 'min' => 3,'message' => UserModule::t("Incorrect username (length between 3 and 20 characters).")),
			array('email', 'email'),
			array('username', 'unique', 'message' => UserModule::t("This user's name already exists.")),
	...
	public function relations()
	{
		$relations = array(
			'profile'=>array(self::HAS_ONE, 'Profile', 'user_id'),
                    'company'=>array(self::HAS_ONE, 'Customers', 'id'),
		);
	...
	public function scopes()
    {
        return array(
            ...
            'notsafe'=>array(
            	'select' => 'id, username, password, email, activkey, createtime, lastvisit, superuser, status, companyid',
            ),
        );
    }
    ...
    {
        return array(
            'select' => 'id, username, email, createtime, lastvisit, superuser, status, companyid',
        );
    }
	public function name() {
		return sprintf('%s %s', $this->email, $this->username);
	}
